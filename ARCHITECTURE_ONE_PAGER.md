...
```
┌─ Memory Store ───────────────────────────────────────────┐
│  Code Index (vector)  │  Review History  │  Memories     │
└──────────────────────────────┬───────────────────────────┘
                               │
                        Context Agent
...
⚡ `--quick` mode: skips Context Agent, runs Correctness + Security only, surfaces
BLOCKING findings in ~30 seconds.
...
**Guardrails:** GitHub and ticket tracker are read-only. `post_review_comment` is
the only write, gated behind the approval UI. File citation check, secret pattern
scan, and scope-creep budget enforced on every run.
