...
create policy "service_role can manage tracked_prs"
  on tracked_prs for all to service_role using (true) with check (true);

create policy "service_role can manage configured_repos"
  on configured_repos for all to service_role using (true) with check (true);
...
create policy "service_role can manage memories"
  on memories for all to service_role using (true) with check (true);

create policy "service_role can manage review_history"
  on review_history for all to service_role using (true) with check (true);

create policy "service_role can manage review_checkpoints"
  on review_checkpoints for all to service_role using (true) with check (true);
