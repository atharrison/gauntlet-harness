...
const RECOMMENDED_ACTIONS: Record<AlarmType, string> = {
  [AlarmType.TURN_LIMIT_EXCEEDED]:
    'Retry with --quick mode; report partial results',
  ...
  [AlarmType.SCHEMA_VALIDATION_FAILED]:
    'Discard run; surface raw model output for debugging',
  [AlarmType.SECRET_DETECTED]:
    'Discard run; do not write to disk; alert engineer',
  [AlarmType.HALLUCINATED_FILE_CITATION]:
    'Strip finding; flag for quality review',
  [AlarmType.REPEATED_TOOL_CALL]: 'Abort loop; likely confused agent state',
  [AlarmType.SCOPE_BUDGET_EXCEEDED]:
    'Continue with gathered context; log for tuning',
  [AlarmType.TOOL_TIMEOUT]:
    'Return error-as-data; agent decides how to proceed',
  [AlarmType.CHECKPOINT_FAILED]:
    'Stop run at that stage; surface checkpoint error to caller',
