...
export function checkPRSize(
  ...
  const maxFiles =
    options.maxFiles ?? parseInt(process.env.PR_MAX_FILES ?? '50', 10)
  const maxLines =
    options.maxLines ?? parseInt(process.env.PR_MAX_LINES ?? '3000', 10)
...
export function validateReviewOutput(
  ...
  for (const finding of allFindings) {
    if (!prFiles.has(finding.file)) {
      ...
      fireAlarm(alarm)
      errors.push(
        `Hallucinated file citation: ${finding.file} is not in the PR diff`
      )
    }
  }
