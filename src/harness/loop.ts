...
import type { ModelClient, Message, ToolDefinition, ToolCall, ModelReply } from "./models";
    ...
    for (const call of reply.toolCalls) {
      ...
      if (sig === lastToolSig) {
        ...
        if (repeatCount >= 3) {
          ...
          fireAlarm(alarm);
          throw new Error(`Repeated tool call detected: ${call.name} called 3× with identical args`);
        }
      } else {
        lastToolSig = sig;
        repeatCount = 1;
