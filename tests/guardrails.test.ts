import { validateReviewOutput, checkPRSize, stripHallucinatedFindings } from "../src/harness/guardrails";
...
describe("validateReviewOutput", () => {
  ...
  it("fails schema validation for malformed output", () => {
    const result = validateReviewOutput({ summary: "incomplete" }, baseContext, "rev-1");
  ...
  it("detects hallucinated file citation and fires alarm", () => {
    ...
    expect(result.pass).toBe(false);
    expect(result.errors.some((e) => e.includes("Hallucinated file citation"))).toBe(true);
  });
  ...
  it("detects secret patterns in review output", () => {
    ...
    expect(result.pass).toBe(false);
    expect(result.errors.some((e) => e.includes("Secret pattern detected"))).toBe(true);
  });
});
...
describe("stripHallucinatedFindings", () => {
  it("removes findings referencing files not in PR", () => {
    const review: PRReview = {
      ...validReview,
      nits: [
        { id: "n1", severity: "NIT", category: "STYLE", file: "src/foo.ts", title: "x", body: "y", confidence: 0.5 },
        { id: "n2", severity: "NIT", category: "STYLE", file: "src/ghost.ts", title: "x", body: "y", confidence: 0.5 },
      ],
    };
